  ...
  \begin{center}
    ...
    \scalebox{0.8}{
      \begin{tabular}{lccc}
        ...
        Force & 8 & ~\ref{force} \\
        Order-Edges & 9 & ~\ref{order_edges} \\
        Order-Edges and Pre-Post & 10 & ~\ref{order_edges} \\
        Order-Edges reversed & 11 & ~\ref{order_edges} \\
        Order-Edges reversed and Pre-Post & 12 & ~\ref{order_edges} \\
        Order-Edges reversed and Pre-Post with slightly more iterations & 13 & ~\ref{order_edges} \\
        Pre-Post & 14 & ~\ref{order-pre-post} \\
        Order-Edges 2 & 15 & ~\ref{order_edges} \\
        Order-Edges 2 reversed & 16 & ~\ref{order_edges}
      \end{tabular}
    }
    \label{algorithms}
...
This observation is supported by the results of Force \enquote{Order Edges} and \enquote{Order Edges Reversed}. The first one places the hyper-edges with less variables at the top, whereas the other one places them at the bottom of the graph. It can be gathered from the results that this statement is true in many cases, where the second one would worsen the computing time by orders of magnitude. We have therefore uncovered one more parameter directly alterating the computation time..

However, in some cases, placing the hyper-edges with many variables at the top has yielded better results. Likewise, placing the first variables of the Petri network in the same order inside the SDD may sometimes prove to be the wrong idea. We have been as of now unable to explain these results that seem to contravene the idea we have of the SDD. That is why further studies must to be conducted to broaden our understanding of the SDD and the way they affect the model-checker. 
