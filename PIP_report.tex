...
\Function {FORCE}{$variables, edges$}
  \Repeat
    \ForAll {$edges$}
      \State {compute its $COG$} \Comment {$COG$: mean value of the variables it connects}
    \EndFor
    \ForAll {$variables$}
      \State {compute its new $position$} \Comment {$position$: mean value of the $COG$s of its connected edges}
    \EndFor
